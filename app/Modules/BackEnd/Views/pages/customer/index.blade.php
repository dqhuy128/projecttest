                ...
                <div class="row">
                    <div class="form-group col-sm-3">
                        <div class="input-group">
                            ...
                            <input type="text" name="name" class="form-control" placeholder="Tên khách hàng" value="{{ $search_data->name }}">
                        </div>
                    </div>
                    <div class="form-group col-sm-3">
                        <div class="input-group">
            ...
            <div class="card-body">
                <table class="table table-bordered table-striped table-responsive">
                    ...
                    <tr>
                        ...
                        <td>
                            <b>{{ $item->name }}</b>
                            <div class="mt-2 font-sm"><i>{{ $item->address }}</i></div>
                        </td>
                        <td align="center">
                            @if($item->image != '')
                                <img src="{{ \ImageURL::getImageUrl($item->avartar, 'customer', 'original') }}" width="100" />
                            @endif
                        </td>
                        ...
                        @if(\Lib::can($permission, 'edit') || \Lib::can($permission, 'delete'))
                            <td align="center">
                                @if(\Lib::can($permission, 'edit'))
                                    <div class="mb-2">
                                        @if($item->status == 2)
                                            <a href="javascript:void(0)" class="text-primary" onclick="shop.admin.updateStatus({{ $item->id }},false,'customer')" title="Đang hiển thị, Click để ẩn"><i class="fe-check-circle"></i></a>
                                        @else
                                            <a href="javascript:void(0)" class="text-secondary" onclick="shop.admin.updateStatus({{ $item->id }}, true,'customer')" title="Đang ẩn, Click để hiển thị"><i class="fe-check-circle"></i></a>
                                        @endif
                                    </div>
