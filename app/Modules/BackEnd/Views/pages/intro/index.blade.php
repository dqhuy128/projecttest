            ...
            <div class="card-body">
                <table class="table table-bordered table-striped table-responsive">
                    ...
                    <tr>
                        ...
                        <td align="center">
                            @if($item->image != '')
                                <img src="{{ \ImageURL::getImageUrl($item->image, 'intro', 'original') }}" width="100" />
                            @endif
                        </td>
                        ...
                        @if(\Lib::can($permission, 'edit') || \Lib::can($permission, 'delete'))
                            <td align="center">
                                @if(\Lib::can($permission, 'edit'))
                                    <div class="mb-2">
                                        @if($item->status == 2)
                                            <a href="javascript:void(0)" class="text-primary" onclick="shop.admin.updateStatus({{ $item->id }},false,'intro')" title="Đang hiển thị, Click để ẩn"><i class="fe-check-circle"></i></a>
                                        @else
                                            <a href="javascript:void(0)" class="text-secondary" onclick="shop.admin.updateStatus({{ $item->id }}, true,'intro')" title="Đang ẩn, Click để hiển thị"><i class="fe-check-circle"></i></a>
                                        @endif
                                    </div>
