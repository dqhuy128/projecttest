...
class Lib{
    ...
    public static function getBreadcrumb($extraCommand = [], $noExtra = false){
        if(!empty(self::$breadcrumb)) {
            $defVal = self::defaultBreadcrumb();
...
//        return '<title>' . ($title == '' ? $host : ($host . ' - ' . $title)) . '</title>';
    ...
    public static function getDefaultLang(){
        if(empty(self::$defLang)){
            self::$defLang = self::getCookie(self::getLanguageKey(), env('APP_LANG'),  true);
        }
        return self::$defLang;
    }
    ...
    public static function priceFormat($price = 0,$currency = 'đ', $isDisplayCurrency = true){
        if($currency ===''){
...
//        part = (snippet || contentDetails || fileDetails || player || processingDetails || recordingDetails || statistics || status || suggestions || topicDetails)
        try{
            if (!empty($id)){
                $id = self::getIDYoutube($id);

                $json_snippet = file_get_contents("https://www.googleapis.com/youtube/v3/videos?part=snippet&id=" . $id . "&key=AIzaSyDyq_F4RWc_005qtiJFxszeQ8wLUkz_xik&fields=items(id,snippet(title,description,channelId,publishedAt,thumbnails))");
                $json_statistics = file_get_contents("https://www.googleapis.com/youtube/v3/videos?part=statistics&id=" . $id . "&key=AIzaSyDyq_F4RWc_005qtiJFxszeQ8wLUkz_xik&fields=items(id,statistics(viewCount,likeCount,dislikeCount,favoriteCount,commentCount))");
                $jsonDataSnippet = json_decode($json_snippet);
                $jsonDataStatistics = json_decode($json_statistics);
                $data_defaul = [];
                $data_defaul['id'] = $jsonDataSnippet->items[0]->id;
                $data_defaul['title'] = $jsonDataSnippet->items[0]->snippet->title;
                $data_defaul['description'] = $jsonDataSnippet->items[0]->snippet->description;
                ...
                $data_defaul ['thumbnails'] = [
                    'default' => isset($thumbnails->default) && !empty($thumbnails->default) ? $thumbnails->default->url : '',
                    'medium' => isset($thumbnails->medium) && !empty($thumbnails->medium) ? $thumbnails->medium->url : '',
                    'high' => isset($thumbnails->high) && !empty($thumbnails->high) ? $thumbnails->high->url : '',
                    'standard' => isset($thumbnails->standard) && !empty($thumbnails->standard) ? $thumbnails->standard->url : '',
                    'maxres' => isset($thumbnails->maxres) && !empty($thumbnails->maxres) ? $thumbnails->maxres->url : '',
                ];
                $count = $jsonDataStatistics->items[0]->statistics;
                $data_defaul['count'] = [
                    'viewCount' => isset($count->viewCount) && !empty($count->viewCount) ? $count->viewCount : 0,
                    'likeCount' => isset($count->likeCount) && !empty($count->likeCount) ? $count->likeCount : 0,
                    'dislikeCount' => isset($count->dislikeCount) && !empty($count->dislikeCount) ? $count->dislikeCount : 0,
                    'favoriteCount' => isset($count->favoriteCount) && !empty($count->favoriteCount) ? $count->favoriteCount : 0,
                    'commentCount' => isset($count->commentCount) && !empty($count->commentCount) ? $count->commentCount : 0,
                ];
                return $data_defaul;
            }
        } catch (\Exception $e) {
            TelegramAlertService::sendMessage($e);
        }
        return '';
    }
...
//            $version = time();
            ...
            switch ($ext){
                case 'js':
                    if($echo && !self::isUrl($src)){
                        $contentJs = file_get_contents(base_path() . '/public/' . $src);
                        $html = '<script type="text/javascript" '.$moreExt.'>'.$contentJs.'</script>';
                    }else {
                        $html = sprintf('<script type="text/javascript" src="'.$link.'?ver='.$version.'"'.$moreExt.'></script>');
                    }
                    
                    break;
                case 'css':
                    if($echo){
                        $contentCss = file_get_contents(base_path() . '/public/' . $src);
...
//                $l['sub'] = (object)[];
    ...
    public static function getIDYoutube($url)
    {
        ...
        $youtube_id = count($match) > 2 ? $match[1] : $url;
        return $youtube_id;
    }
}