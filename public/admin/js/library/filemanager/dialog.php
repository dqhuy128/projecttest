...
$config['current_path']=$config['current_path'].'/';
$url=$config['base_url'];
...
$_SESSION['idFilter']=$_GET['idFilter'];
...
<body>
    ...
    <input type="hidden" id="lang_duplicate" value="<?php echo trans('Chọn ảnh này');?>" />
    <input type="hidden" id="choosed_image" value="<?php echo trans('Chọn ảnh này Real');?>" />
    ...
    if ($ftp) {
        ...
        if ($file != "." && $file != "..") {
            if (is_dir($config['current_path'] . $rfm_subfolder . $subdir . $file)) {
                $date = ($config['current_path'] . $rfm_subfolder . $subdir . $file);
                $current_folders_number++;
...
<script>
    ...
    <?php if ( $config['tui_active'] ) { ?>
        ...
        if (image_editor) { 
            ...
            var dataImg = [];
            $(function() {
                sessionStorage.setItem('dataImg', JSON.stringify(dataImg));
            });
        
            $(document).on('click', '.upload-btn', function() { 
                id_filter=sessionStorage.getItem("id_filter");
                var new_id_filter = id_filter.replace(/,/g, '');
                timePM ="<?php echo time()?>";
                <?php $_SESSION['timePM']=time()?>
                id_filter=sessionStorage.setItem("timePM",timePM);
            });
            $(document).on('click', '.context-menu-list', function() { 
                id_filter=sessionStorage.getItem("id_filter");
                var new_id_filter = id_filter.replace(/,/g, '');
                timePM = Date.now();timePM=Math.round(timePM / 1000);
                $('.input-block-level').val(new_id_filter+'-0-'+timePM);
                $('.btn-primary:contains(OK)').click();
            });
            $(document).on('click', '.btn-primary:contains(OK)', function() { 
                id_filter=sessionStorage.getItem("id_filter");
                var new_id_filter = id_filter.replace(/,/g, '');
                fileName = new_id_filter+'-0-'+timePM;
                sessionStorage.setItem("fileName",fileName);

                tab_id= sessionStorage.getItem("tab_id");
                <?php
                    $idFilterNew=str_replace(",","",$_SESSION['idFilter']);
                    $idFilterNew=str_replace("'","",$idFilterNew); 
                    if(!isset($_SESSION['img'])){
                        $_SESSION['img'][0]=$idFilterNew.'-0-'.$_SESSION['timePM'];
                    }else{
                        array_push($_SESSION['img'],$idFilterNew.'-0-'.$_SESSION['timePM']);
                    }
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo $url ?>"+'/admin/ajax/product/file_manager',
                    headers: {
                        'X-CSRF-TOKEN': sessionStorage.getItem("token")
                    },
                    data: {idFilter:sessionStorage.getItem("id_filter"),
                            prd_id: sessionStorage.getItem("prd_id"),timePM
                            }
                }).done(function( msg ) {
            ...
            function update(value){
                dataImg =JSON.parse(sessionStorage.getItem('dataImg'));
                dataImg.push(value);
                sessionStorage.setItem('dataImg', JSON.stringify(dataImg));
            }
