...
function duplicate_file( $old_path, $name, $ftp = null, $config = null )
{
	...
	$new_path = '../../../../upload/products/original' . "/" . $name . "." . $info['extension'];
	if($ftp){
		try{
...
/**
 * Return current file when choosed
 *
 * @param  string  $old_path
 * @param  string  $name      New file name without extension
 *
 * @return  bool
 */
function choosed_iamge( $old_path, $name, $ftp = null, $config = null )
{
    $info = pathinfo($old_path);
...
function rename_file($old_path, $name, $ftp = null, $config = null)
{
	...
	if($ftp){
		...
		if (file_exists($old_path) && is_file($old_path))
		{
			...
			print_r(rename($old_path, $new_path));
...
function rcopy($source, $destination, $is_rec = false)
{
	...
	else
	{
		if (file_exists($source))
		{
			...
			{
				$pinfo = pathinfo($source);
				$dest2 = rtrim($destination, '/') . DIRECTORY_SEPARATOR . 'test.jpg';
			}
			else
			{
				$dest2 = $destination;
